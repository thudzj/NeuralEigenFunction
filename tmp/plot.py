...
def draw_eigenvalues(eigenval_gd, eigenval_nystrom, eigenval_our):
	...
	axins.plot(np.array(list(range(10))) + 0.05, eigenval_gd[:10] + 0.002, marker='o', markersize=5)
	axins.plot(np.array(list(range(10))) + 0.05, eigenval_nystrom[:10] + 0.002, '--*', markersize=6)
	axins.plot(np.array(list(range(10))) + 0.05, eigenval_our[:10] + 0.002, ':v', markersize=6)
...
def draw_kernel(kernels):
	labels = ['Ground truth', 'The Nyström method ($k=10$)', 'Our ($k=10$)', 'Random feature approach ($S=10$)', ]
	ma = 1
	mi = -1
